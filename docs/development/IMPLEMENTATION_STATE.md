...
- Phase 0C connection-metadata-aware HTTPS routing is implemented and CI-verified:
  - `ModelRoute.connection_id` remains the sole reference to connection-owned endpoint configuration;
  - when `require_https` is enabled, gateway routes resolve their persisted connection metadata during admissibility;
  - gateway routes require an active, same-account `GatewayConnection` with a syntactically valid HTTPS URL and no URL userinfo;
  - missing resolver/metadata, wrong connection type, inactive status, cross-account ownership, malformed URL, HTTP, and URL credentials all fail closed;
  - provider routes remain independent of gateway endpoint metadata;
  - manual overrides reuse the exact same admissibility path and cannot bypass HTTPS/connection checks;
  - routing remains planning-only and introduces no provider/gateway execution or spend.
- Current full CI test inventory: 135 unique tests (contracts 6, domain 33, MCP server 31, security 65).
...
1. Phase 0F — Apps SDK/MCP UI.
2. Phase 0G — synthetic repository-backed vertical slice.
3. Phase 1 — minimal product orchestration.
